...
const AdvertisementController = {
  ...
  createAdvertisement: async (req, res) => {
    try {
      ...
      if (isNaN(formattedPrice)) {
        return res
          .status(400)
          .json(formatErrorResponse("Invalid price format"));
      }
  ...
  getAllAdvertisements: async (req, res) => {
    ...
    try {
      ...
      const ads = await AdvertisementModel.getAllAdvertisementsWithPagination(
        limitInt,
        offset
      );
      ...
      return res
        .status(200)
        .json(
          formatSuccessResponse(response, "Advertisements fetched successfully")
        );
    } catch (error) {
      return res
        .status(500)
  ...
  addToFavorites: async (req, res) => {
    ...
    try {
      const success = await AdvertisementModel.addFavorite(
        userId,
        advertisementId
      );
      if (success) {
        return res
          .status(200)
          .json(
            formatSuccessResponse(null, "Advertisement added to favorites")
          );
      } else {
        return res
          .status(400)
          .json(formatErrorResponse("Advertisement is already a favorite"));
      }
    } catch (error) {
      ...
      return res
        ...
        .json(
          formatErrorResponse(
            "Error adding advertisement to favorites",
            error.message
          )
        );
    }
  ...
  removeFromFavorites: async (req, res) => {
    ...
    const userId = req.user?.userId; // Assuming user ID is available in req.user
    try {
      const success = await AdvertisementModel.removeFavorite(
        userId,
        advertisementId
      );
      if (success) {
        return res
          .status(200)
          .json(
            formatSuccessResponse(null, "Advertisement removed from favorites")
          );
      } else {
        return res
          .status(404)
          .json(formatErrorResponse("Advertisement not found in favorites"));
      }
    } catch (error) {
      ...
      return res
        ...
        .json(
          formatErrorResponse(
            "Error removing advertisement from favorites",
            error.message
          )
        );
    }
  ...
  getFavorites: async (req, res) => {
    ...
    try {
      ...
      console.log("Favorite advertisements:", favoriteAds);
      ...
      return res
        ...
        .json(
          formatSuccessResponse(
            favoriteAds,
            "Favorite advertisements retrieved successfully"
          )
        );
    } catch (error) {
      ...
      return res
        ...
        .json(
          formatErrorResponse(
            "Error retrieving favorite advertisements",
            error.message
          )
        );
    }
  ...
  rateAdvertisement: async (req, res) => {
    ...
    console.log(
      `Adding rating for advertisementId: ${advertisementId}, userId: ${userId}, rating: ${rating}`
    );

    try {
      // Insert rating into the database
      const newRating = await AdvertisementModel.addRating(
        advertisementId,
        userId,
        rating
      );
      const averageRating = await AdvertisementModel.getAverageRating(
        advertisementId
      );
  ...
  getRateAdvertisement: async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Error getting advertisement rating" });
    }
  },
};
