...
const StoreController = {
  ...
  createStore: async (req, res) => {
    ...
    // Construct image URL
    const imageUrl = image ? `http://${process.env.VPS_IP}:${process.env.PORT}/uploads/stores/${image.filename}` : null;
    
    // Get URLs for uploaded files
    const fileUrls = files.map(file => `http://${process.env.VPS_IP}:${process.env.PORT}/uploads/stores/${file.filename}`);
  ...
  getAllStores: async (req, res) => {
    try {
      ...
      res.status(200).json(formatSuccessResponse(stores, 'Stores retrieved successfully'));
    } catch (error) {
      res.status(500).json(formatErrorResponse(error.message));
    }
  },
