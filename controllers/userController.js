...
const UserController = {
  ...
  createUser: async (req, res) => {
    ...
    if (phone.length > 50) {
      return res.status(400).json(formatErrorResponse('Phone number is too long'));
    }
    if (email.length > 100) {
    ...
    try {
      ...
      if (existingUser) {
        return res.status(409).json(formatErrorResponse('Email is already in use'));
      }
      ...
      res.status(201).json(formatSuccessResponse({ user, accessToken, refreshToken }, 'User created successfully'));
    } catch (error) {
  ...
  getUserByPhone: async (req, res) => {
    ...
    try {
      ...
      if (user) {
        res.status(200).json(formatSuccessResponse(user, 'User retrieved successfully'));
      } else {
  ...
  loginUser: async (req, res) => {
    ...
    try {
      if (!phone || !password) {
        return res.status(400).json(formatErrorResponse('Phone number and password are required'));
      }
      ...
      if (user && user.password && await bcrypt.compare(password, user.password)) {
        const accessToken = generateAccessToken(user);
        const refreshToken = generateRefreshToken(user);
        res.status(200).json(formatSuccessResponse({ accessToken, refreshToken }, 'Login successful'));
      } else {
        res.status(401).json(formatErrorResponse('Invalid phone number or password'));
      }
    } catch (error) {
      Sentry.captureException(error);  // Capture error with Sentry
      res.status(500).json(formatErrorResponse(error.message));
  ...
  resetPassword: async (req, res) => {
    ...
    try {
      ...
      if (!accessToken) {
        return res.status(400).json(formatErrorResponse("Access token is required"));
      }

      const decoded = jwt.verify(accessToken, process.env.ACCESS_TOKEN_SECRET);
      const user = await UserModel.getUserById(decoded.userId);
      if (!user) {
        return res.status(400).json(formatErrorResponse("Invalid or expired token"));
      }
  ...
  forgotPassword: async (req, res) => {
    ...
    try {
      ...
      res.status(200).json(formatSuccessResponse({ accessToken }, "Password reset token sent successfully"));
    } catch (error) {
      Sentry.captureException(error);  // Capture error with Sentry
      res.status(500).json(formatErrorResponse(error.message));
    }
  ...
  createCustomToken: (req, res) => {
    ...
    try {
      ...
      res.status(200).json(formatSuccessResponse({ token }, 'Custom token created successfully'));
    } catch (error) {
      Sentry.captureException(error);  // Capture error with Sentry
      res.status(500).json(formatErrorResponse(error.message));
    }
